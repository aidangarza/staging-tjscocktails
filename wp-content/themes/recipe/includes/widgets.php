		...
		if( $recipes->have_posts() ){
			...
			while( $recipes->have_posts() ){
				...
				switch( $instance['orderby'] ){
					case '': $subtitle = '<i class="fa fa-calendar-o tip" data-title="'.__( 'Create date', 'recipe' ).'"></i>'.get_the_time( 'F j, Y' ); break;
					case 'average_review': 
						ob_start();
						...
						ob_end_clean();
						break;
					case 'title': $subtitle = '<i class="fa fa-calendar-o tip" data-title="'.__( 'Create date', 'recipe' ).'"></i>'.get_the_time( 'F j, Y' ); break;
					case 'favourited': 
					...
					case 'views': 
						...
						$subtitle = '<i class="fa fa-eye tip" data-title="'.__( 'Viewes', 'recipe' ).'"></i>'.$views; break;
				}
				echo    '<li class="top-authors">
							<div class="widget-image-thumb">
								<img src="'.esc_url( $image[0] ).'" class="img-responsive" width="60" height="60" alt=""/>
							</div>
			...
			foreach( $instance['categories'] as $category_id ){
				...
				if( $term ){
					echo '<li>
							...
							<a href="'.( esc_url( add_query_arg( array( 'recipe-category' => $term->slug ), $permalink ) ) ).'">
								'.$term->name.'
							</a>
						  </li>';				
				}
	...
	function form( $instance ) {
		...
		<p>
			...
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id('categories') ); ?>" name="<?php echo esc_attr( $this->get_field_name('categories') ); ?>[]" multiple>
				<?php
				$recipe_categories = get_terms( 'recipe-category' );
