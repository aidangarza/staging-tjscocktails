...
if( $page == 'edit_recipe' ){
    ...
    $recipe_category = array_shift( $recipe_category );
    $recipe_category = $recipe_category->slug;
    $recipe_cuisine = get_the_terms( $recipe_id, 'recipe-cuisine' );
    $recipe_cuisine = array_shift( $recipe_cuisine );
    $recipe_cuisine = $recipe_cuisine->slug;
...
?>
    ...
    <p><?php _e( 'Populate fields bellow and after review yout recipe will be approved if everything checks out', 'recipe' ) ?></p>
<?php
}
else{
...
<form method="post">
    ...
    <div class="form-group">
        <label for="featured_image"><?php _e( 'Featured Image *', 'couponxl' ); ?></label>
        ...
        <a href="javascript:;" class="image-upload featured-image"><?php _e( 'Select Image', 'couponxl' ) ?></a>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" name="recipe_video" id="recipe_video" value="<?php echo esc_attr( $recipe_video ) ?>" class="form-control">
    </div> 

    <div class="form-group">
        <label for="recipe_images"><?php _e( 'Recipe Images', 'couponxl' ); ?></label>
        ...
        <a href="javascript:;" class="image-upload recipe-images"><?php _e( 'Select Images', 'couponxl' ) ?></a>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" name="recipe_prep_time" id="recipe_prep_time" value="<?php echo esc_attr( $recipe_prep_time ) ?>" class="form-control">
    </div>

    <div class="form-group">
        <label for="recipe_cook_time"><?php _e( 'Time To Cook *', 'recipe' ); ?></label>
        <input type="text" name="recipe_cook_time" id="recipe_cook_time" value="<?php echo esc_attr( $recipe_cook_time ) ?>" class="form-control">
    </div>

    <div class="form-group">
        <label for="recipe_ready_in"><?php _e( 'Ready Time *', 'recipe' ); ?></label>
        <input type="text" name="recipe_ready_in" id="recipe_ready_in" value="<?php echo esc_attr( $recipe_ready_in ) ?>" class="form-control">
    </div>

    <div class="form-group">
        <label for="recipe_ingredient"><?php _e( 'Ingredients. To add a title wrap it in # for example #For Pudding# *', 'recipe' ); ?></label>
        <textarea class="form-control" name="recipe_ingredient" id="recipe_ingredient"><?php echo $recipe_ingredient; ?></textarea>
    </div>
    ...
    <div class="form-group">
        <label for="recipe_steps"><?php _e( 'Steps ( separated with -- ) *', 'recipe' ); ?></label>
    ...
    <div class="form-group">
        ...
        <select name="recipe_category" id="recipe_category" class="form-control">
            <option value=""><?php _e( '- Select -', 'recipe' ) ?></option>
            <?php
            $terms = get_terms( 'recipe-category', array( 'hide_empty' => false ) );
            if( !empty( $terms ) ){
    ...
    <div class="form-group">
        ...
        <select name="recipe_cuisine" id="recipe_cuisine" class="form-control">
            <option value=""><?php _e( '- Select -', 'recipe' ) ?></option>
            <?php
            $terms = get_terms( 'recipe-cuisine', array( 'hide_empty' => false ) );
            if( !empty( $terms ) ){
                foreach( $terms as $term ){
